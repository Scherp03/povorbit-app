...
import {
  Play, Square, Trophy, Timer, MapPin, ChevronLeft,
  Wifi, WifiOff, Loader, Zap, User, RotateCcw, Navigation
} from "lucide-react";
...
export default function PovOrbit() {
  ...
  const handlePosition = (pos) => {
    ...
    if (movedAwayRef.current && distFromStart < 25) {
      ...
      setLaps((prev) => {
        const next = prev + 1;
        setTimeout(() => commitPlayer(name, next, stateRef.current.totalDist, stateRef.current.walkTime), 50);
        return next;
      });
      triggerFlash();
    }
  ...
  const simulateLap = () => {
    ...
    setTotalDist((prev) => {
      ...
      setLaps((prev2) => {
        const nl = prev2 + 1;
        setTimeout(() => commitPlayer(name, nl, nd, stateRef.current.walkTime), 50);
        return nl;
      });
      return nd;
    });
  ...
  const enterSession = () => {
    ...
    if (existing) {
      ...
      stateRef.current = { laps: existing.laps, totalDist: d, walkTime: existing.timeSeconds };
    }
    setScreen("tracker");
  };
          ...
          {players.length > 0 && (
            <div style={{ marginTop: 48 }}>
              <p className="po-label">
                <Trophy size={10} style={{ display: "inline", marginRight: 6 }} />
                Current Leaders
              </p>
              {players.slice(0, 3).map((p, i) => (
                <div key={p.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                  <span style={{ fontFamily: "var(--font-ui)", fontSize: 14, color: i === 0 ? "#ffd700" : i === 1 ? "#c0c0c0" : "#cd7f32" }}>
                    {["🥇", "🥈", "🥉"][i]} {p.name}
                  </span>
                  <span style={{ fontFamily: "var(--font-ui)", fontSize: 14, fontWeight: 700, color: "var(--red-bright)" }}>
                    {p.laps} laps
                  </span>
                </div>
              ))}
      ...
      <div className="po-app">
        <div className="po-screen">
          <div className="po-topbar">
            <button className="po-btn-ghost" onClick={() => setScreen("tracker")}>
            ...
            <>
              ...
              {players.map((p, i) => (
                <div key={p.name} className={`po-board-row${p.name === name ? " highlight" : ""}`}>
                  <div className={`po-board-rank${i === 0 ? " gold" : i === 1 ? " silver" : i === 2 ? " bronze" : ""}`}>
                    {i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}
                  </div>
  ...
  // ── Tracker screen ──
  const gpsColors = { idle: "", acquiring: "acquiring", active: "active", error: "error" };
  const gpsLabel = { idle: "GPS OFF", acquiring: "Acquiring…", active: "GPS LIVE", error: "GPS Error" };
      ...
      <div className="po-screen">
        ...
        <div className="po-topbar">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div className={`po-gps-badge ${gpsColors[gpsStatus] || "idle"}`}>
              <GpsIcon size={10} className={gpsStatus === "acquiring" ? "spin" : ""} />
              {gpsLabel[gpsStatus]}
            </div>
            <button className="po-btn-ghost" onClick={() => setScreen("board")}>
              <Trophy size={12} />
              Board
            </button>
          </div>
        ...
        <div className="po-action-wrap">
          ...
          <div className="po-dev-row">
            ...
            {!isTracking && laps > 0 && (
              <button className="po-dev-btn" onClick={() => {
                setLaps(0); setTotalDist(0); setWalkTime(0);
                stateRef.current = { laps: 0, totalDist: 0, walkTime: 0 };
              }}>
